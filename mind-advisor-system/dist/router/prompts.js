...
exports.buildDirectPrompt = buildDirectPrompt;
exports.buildMultiPrompt = buildMultiPrompt;
exports.buildTemplateCreationPrompt = buildTemplateCreationPrompt;
exports.buildResearchBasedPrompt = buildResearchBasedPrompt;
const time_1 = require("../utils/time");
const formatAdvisor = (advisor) => `- ${advisor.emoji} ${advisor.name}: ${advisor.metadata.specialties.join(', ')} (tone: ${advisor.metadata.tone})`;
function buildSynthesisPrompt(project, advisors, userInput) {
...
Project context:
- Phase: ${project.phase}
- Days since last ship: ${(0, time_1.daysSince)(project.lastShipDate)}
...
function buildDirectPrompt(advisor, project, userInput) {
...
function buildMultiPrompt(advisors, userInput) {
    const lineItems = advisors
        .map((advisor) => `${advisor.emoji} ${advisor.name}: ${advisor.metadata.specialties.join(', ')} | Tone: ${advisor.metadata.tone}`)
        .join('\n');
...
function buildTemplateCreationPrompt(templateName, params, template) {
...
function buildResearchBasedPrompt(params, research) {
